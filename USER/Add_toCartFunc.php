...
if (!isset($_POST['course_id']) || !isset($_POST['course_name']) || !isset($_POST['course_price'])) {
    echo json_encode([
        'success' => false, 
...
$course_name = trim($_POST['course_name']); // Sanitize course name

// ✅ Ensure values are valid
if ($course_id <= 0 || $course_price <= 0 || empty($course_name)) {
    echo json_encode([
        'success' => false, 
...
$query = "INSERT INTO course_cart (usersid, course_id, course_name, course_price, created_at, status) 
          VALUES (?, ?, ?, ?, NOW(), 2)";

$stmt = $conn->prepare($query);
$stmt->bind_param("iisd", $usersid, $course_id, $course_name, $course_price);
